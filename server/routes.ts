...
r.post("/blog-posts", async (req, res) => {
  try {
    ...
    const post: BlogPost = {
      ...
      isPublished: req.body.isPublished ?? true,   // default published so post is visible
...
r.delete("/weekly-assignments/:id/tasks/:taskId", async (req, res) => {
  try {
    ...
    const before = (assignment.tasks ?? []).length;
    assignment.tasks = (assignment.tasks ?? []).filter((t: any) => t.id !== req.params.taskId);

    if (assignment.tasks.length === before)
      return res.status(404).json({ error: "Task not found" });

    await writeJsonFile("weekly-assignments.json", waFile, `Delete task from: ${assignment.projectName}`);
    res.json({ success: true });
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
