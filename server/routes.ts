...
// ALL endpoints required by the frontend are included here.
...
export function registerRoutes(
  ...
  router.post("/auth/login", async (req, res) => {
    try {
      ...
      if (file) { file.lastUpdated = new Date().toISOString();
  ...
  router.post("/engineer-credentials/initialize", async (req, res) => {
    try {
      ...
      for (const eng of (ml?.engineers ?? [])) {
        ...
        if (!usernames.has(u)) {
          file.engineers.push({ id:eng.id, name:eng.name, username:u, password:"drb@123",
      ...
      if (!usernames.has("admin")) {
        file.engineers.push({ id:"admin-1", name:"Admin", username:"admin", password:"admin@drb",
          role:"admin", isActive:true, createdAt:new Date().toISOString() });
        created++;
      }
  ...
  router.get("/engineers-master", async (_req, res) => {
    try {
      ...
      res.json(list.filter(e => { const k=e.name.trim().toLowerCase(); return seen.has(k)?false:(seen.add(k),true); }));
    } catch(e:any) { res.status(500).json({ error:e.message }); }
  });
  ...
  router.put("/engineers-master-list", async (req, res) => {
    try {
      ...
      if (!Array.isArray(engineers)) return res.status(400).json({ message:"Engineers must be an array" });
  ...
  router.get("/engineer-daily-tasks", async (req, res) => {
    try {
      ...
      res.json(engineers.map(eng => {
        const e = entries.find(x => x.engineerName.trim().toLowerCase() === eng.name.trim().toLowerCase());
        return {
          engineerName:eng.name,
          planned:e?.targetTasks?.length ?? 0, completed:e?.completedActivities?.length ?? 0,
  ...
  router.get("/daily-activities", async (req, res) => {
    try {
      ...
      (f?.engineerDailyData ?? []).filter(e => e.date === date).forEach(entry => {
        const k = entry.engineerName.trim().toLowerCase();
        if (!map.has(k)) { map.set(k, { ...entry, targetTasks:[...(entry.targetTasks??[])], completedActivities:[...(entry.completedActivities??[])] }); }
        else {
          const ex = map.get(k)!;
          const ti = new Set(ex.targetTasks.map(t=>t.id)), ai = new Set(ex.completedActivities.map(a=>a.id));
  ...
  router.get("/pending-tasks/:engineer", async (req, res) => {
    try {
      ...
      res.json((f?.engineerDailyData ?? [])
        .filter(e => e.engineerName.trim().toLowerCase()===engineer.trim().toLowerCase() && e.date<today)
  ...
  router.post("/weekly-assignments/:id/tasks", async (req, res) => {
    try {
      ...
      const task: WeeklyAssignmentTask = { id:`task-${Date.now()}`, taskName, targetDate, completionDate, status:status||"not_started" };
  ...
  router.get("/project-names", async (_req, res) => {
    try {
      ...
      const add = (name: string) => {
        ...
        if (!seen.has(k) || name.trim().length > seen.get(k)!.trim().length) seen.set(k, name.trim());
      };
  ...
  router.get("/stats", async (_req, res) => {
    try {
      ...
      res.json({ totalProjects:total, activeMembers:(cf?.engineers??[]).filter(e=>e.isActive&&e.role!=="admin").length,
        completionRate:total>0?Math.round(completed/total*100):0, recentActivities:0 });
    } catch(e:any) { res.status(500).json({ error:e.message }); }
  });
  ...
  router.post("/notifications", (req, res) => { const n={id:`n-${Date.now()}`,...req.body}; notifs.push(n); res.status(201).json(n); });
  router.patch("/notifications/:id/read", (req, res) => { const n=notifs.find(x=>x.id===req.params.id); if(n) n.read="true"; res.json({success:true}); });
  router.patch("/notifications/read-all", (_req, res) => { notifs.forEach(n=>n.read="true"); res.json({success:true}); });
  ...
  router.get("/analytics/engineer-workload", async (_req, res) => {
    try {
      ...
      const resolve = (raw: string) => {
        const c = raw.trim();
        const ex = master.find(e => e.name.trim()===c); if (ex) return ex.name;
        const cl = c.replace(/\s*\([^)]*\)\s*/g,"").trim().toLowerCase();
        return master.find(e => e.name.replace(/\s*\([^)]*\)\s*/g,"").trim().toLowerCase()===cl)?.name ?? c;
      };
      ...
      for (const a of (wf?.assignments??[])) {
        ...
        for (const eng of engs) {
          ...
          if (!pm.has(pk)||a.projectName.trim().length>pm.get(pk)!.projectName.trim().length)
            pm.set(pk,{projectName:a.projectName.trim(),status:STATUS_LABEL[a.currentStatus]||a.currentStatus,
      ...
      res.json({
        ...
        engineers, totalEngineers:engineers.length, totalAssignments:engineers.reduce((s,e)=>s+e.projectCount,0),
      });
    } catch(e:any) { res.status(500).json({ error:e.message }); }
  });
  ...
  router.get("/projects-master", async (_req, res) => {
    try {
      ...
      const seen = new Set<string>();
      res.json(list.filter(p=>{const k=p.projectNumber.trim().toLowerCase();return seen.has(k)?false:(seen.add(k),true);}));
    } catch(e:any) { res.status(500).json({ error:e.message }); }
  });
  ...
  router.get("/projects-overview", async (_req, res) => {
    try {
      ...
      const masterNames = new Set(engs.map(e=>e.name.trim().toLowerCase()));
      ...
      for (const a of data) {
        ...
        if (masterNames.has(eng.toLowerCase())&&!e.engineers.some(x=>x.toLowerCase()===eng.toLowerCase())) e.engineers.push(eng);
        if (a.endDate>e.latestEnd) e.latestEnd=a.endDate;
        if (a.status==="In Progress") e.status="In Progress";
      }
      res.json(Array.from(pm.values()).map(p=>({...p,registeredInMaster:p.projectNumber?validateProjectNumber(p.projectNumber,ml):false})));
    } catch(e:any) { res.status(500).json({ error:e.message }); }
  });
