...
interface EngineerCredential {
  ...
  name: string;
  username: string;
  role: 'admin' | 'engineer' | 'hr' | 'pic' | 'scm';
  company?: string;
  isActive: boolean;
  createdAt: string;
  lastLogin?: string;
...
const ROLE_LABELS: Record<EngineerCredential['role'], string> = {
  admin: 'Admin',
  engineer: 'Engineer',
  hr: 'HR',
...
export default function EngineerManagement() {
  ...
  const { isAdmin, isFullAccessViewer } = useAuth();
  ...
  const [formData, setFormData] = useState({
    ...
    username: "",
    password: "",
    role: "engineer" as EngineerCredential['role'],
    company: "",
    isActive: true,
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        ...
        {!isAdmin && (
          <div className="mb-4 rounded-lg border border-amber-500/30 bg-amber-500/5 px-4 py-2 text-sm text-amber-700 dark:text-amber-300">
            View-only access — you can see all engineer credentials but can't add, edit, or delete them.
          </div>
        )}
      ...
      <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
        <DialogContent data-testid="dialog-add-engineer">
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label>Role</Label>
              <Select value={formData.role} onValueChange={(v: EngineerCredential['role']) => setFormData({ ...formData, role: v })}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="pic">Project In-Charge</SelectItem>
