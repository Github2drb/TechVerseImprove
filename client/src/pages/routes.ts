...
interface EngineerCredential {
  ...
  role: "admin"|"engineer"|"stores"|"documentation"; company?: string;
  mobile?: string;
  email?: string;
...
// ── Documentation Engineer: alert when material tracker is not filled ────────
async function checkDocEngineerMaterialTracker(eng: EngineerCredential): Promise<void> {
  if (eng.role !== "documentation" || !eng.email || eng.isActive === false) return;
  try {
    const [waFile, matFile] = await Promise.all([
      readJsonFile<WAFile>("weekly-assignments.json"),
      readJsonFile<{ projects: Record<string, { projectName: string; materials: unknown[] }> }>("material-tracker.json"),
    ]);
    const assignments = (waFile?.assignments ?? []).filter(a =>
      !(a.currentStatus || "").toLowerCase().includes("complet") &&
      matchEngineer(a.engineerName, eng.name)
    );
    const projects = new Map<string, string>(); // lowercase key -> display name
    for (const a of assignments) {
      const k = a.projectName.trim().toLowerCase();
      if (k && !projects.has(k)) projects.set(k, a.projectName.trim());
    }
    for (const [key, projectName] of projects) {
      const entry = matFile?.projects?.[key];
      const isEmpty = !entry || !Array.isArray(entry.materials) || entry.materials.length === 0;
      if (!isEmpty) continue;
      const html = `<div style="font-family:sans-serif;max-width:600px;margin:auto;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden"><div style="background:#dc2626;color:white;padding:16px 20px"><h2 style="margin:0">📋 Material Tracker Not Filled</h2></div><div style="padding:20px"><p>Dear <b>${eng.name}</b>,</p><p>The Material Procurement Tracker for project <b>${projectName}</b> is <b>${!entry ? "not created yet" : "empty (no materials added)"}</b>.</p><p>As the Documentation Engineer, please fill in the material details (BOM, PR, PO, receipt targets) for this project.</p><a href="https://drbtechverse.in/material-tracker" style="display:inline-block;background:#dc2626;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;margin-top:12px">Open Material Tracker</a></div></div>`;
      await sendEmail(eng.email, `📋 Material Tracker Pending — ${projectName}`, html);
      console.log(`[docEngineerAlert] Mailed ${eng.email} for project: ${projectName}`);
    }
  } catch (e: any) { console.error("[checkDocEngineerMaterialTracker]", e.message); }
}

async function runDocumentationDailyCheck(): Promise<void> {
  try {
    const f = await readJsonFile<CredFile>("engineers_auth.json");
    const docs = (f?.engineers ?? []).filter(e => e.role === "documentation" && e.isActive !== false && e.email);
    for (const eng of docs) await checkDocEngineerMaterialTracker(eng);
  } catch (e: any) { console.error("[runDocumentationDailyCheck]", e.message); }
}

let docCheckScheduled = false;
function scheduleDocumentationDailyCheck(): void {
  if (docCheckScheduled) return;
  docCheckScheduled = true;
  const schedule = () => {
    const now = new Date();
    const next = new Date(now);
    next.setUTCHours(3, 30, 0, 0); // 03:30 UTC = 09:00 IST
    if (next.getTime() <= now.getTime()) next.setUTCDate(next.getUTCDate() + 1);
    setTimeout(() => { runDocumentationDailyCheck().catch(console.error); schedule(); }, next.getTime() - now.getTime());
  };
  schedule();
}
...
export function registerRoutes(httpServer: Server, app: ReturnType<typeof import("express")["default"]>) {
  ...
  // Daily 9:00 AM IST check: mail Documentation Engineers for projects with empty material tracker
  scheduleDocumentationDailyCheck();
  ...
  r.post("/engineer-credentials", async (req, res) => {
    try {
      ...
      // Documentation Engineer assigned → check material trackers of their projects (background)
      if (eng.role === "documentation") checkDocEngineerMaterialTracker(eng).catch(console.error);
  ...
  r.put("/engineer-credentials/:id", async (req, res) => {
    try {
      ...
      const prevRole = f.engineers[i].role;
      const body = { ...req.body };
      if (!body.password) delete body.password; // blank = keep current password
      else body.password = await bcrypt.hash(body.password, 10);
      f.engineers[i]={...f.engineers[i],...body,id:req.params.id}; f.lastUpdated=new Date().toISOString();
      await writeJsonFile("engineers_auth.json",f,`Update engineer ${req.params.id}`);
      // Newly assigned as Documentation Engineer → check material trackers of their projects (background)
      if (f.engineers[i].role === "documentation" && prevRole !== "documentation")
        checkDocEngineerMaterialTracker(f.engineers[i]).catch(console.error);
...
// v2026-07-11: documentation-engineer material tracker alerts
