...
interface MaterialStatus {
  ...
  daysLeft: number | null; // days until target receipt (negative = overdue), null if no target/received
}
function getMaterialStatus(m: MaterialRow): MaterialStatus {
  ...
  return {
    ...
    daysLeft: received ? null : dleft,
  };
}
// Human-readable "days left" / "overdue by" label for the Target Receipt date
function daysLeftLabel(status: MaterialStatus): string | null {
  if (status.received) return null;
  if (status.daysLeft === null) return null;
  if (status.daysLeft < 0) return `Overdue by ${Math.abs(status.daysLeft)} day${Math.abs(status.daysLeft) === 1 ? "" : "s"}`;
    ...
    <div className={`rounded-xl border p-4 space-y-3 transition-colors ${
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
        ...
        <div>
          ...
          {daysLeftLabel(status) && (
            <span className={`text-[10px] font-medium ${status.receiptOverdue ? "text-red-500" : status.receiptDueSoon ? "text-amber-500" : "text-muted-foreground"}`}>
              {daysLeftLabel(status)}
            </span>
          )}
        </div>
...
export default function MaterialProcurementTracker() {
  ...
  const { canEditMaterials } = useAuth();
  ...
  // Materials whose Target Receipt date has passed with no Actual Receipt yet,
  // for the "overdue for receipt" table below — scoped to the selected project.
  const overdueMaterials = useMemo(() => {
    return materials
      .map(m => ({ m, status: getMaterialStatus(m) }))
      .filter(({ status }) => status.receiptOverdue);
  }, [materials]);
      ...
      <div className="mx-auto px-4 py-6 w-full max-w-[1400px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="grid gap-2">
                ...
                <Input
                  ...
                  disabled={!selectedProject || !canEditMaterials}
                />
              </div>
            </div>
          </CardContent>
        ...
        {!selectedProject ? (
          ...
          <>
            ...
            {/* Materials overdue for Actual Receipt — scoped to this project */}
            {overdueMaterials.length > 0 && (
              <Card className="mb-6 border-red-500/40">
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm flex items-center gap-2 text-red-500">
                    <Bell className="h-4 w-4"/>
                    Materials Overdue for Receipt — {selectedProject} ({overdueMaterials.length})
                  </CardTitle>
                </CardHeader>
                    ...
                    <table className="w-full text-xs">
                      <thead>
                        <tr className="border-b text-left text-muted-foreground">
                          ...
                          <th className="py-1.5 pr-3 font-medium">Target Receipt</th>
                          <th className="py-1.5 pr-3 font-medium">Days Overdue</th>
                          <th className="py-1.5 pr-3 font-medium">Notes</th>
                        </tr>
                      </thead>
                      <tbody>
                        {overdueMaterials.map(({ m, status }) => (
                          <tr key={m.id} className="border-b last:border-0">
                            <td className="py-1.5 pr-3 font-medium">{m.name || "—"}</td>
                            <td className="py-1.5 pr-3">{m.qty} {m.unit}</td>
                            <td className="py-1.5 pr-3">{fmtDate(m.targetReceipt)}</td>
                            <td className="py-1.5 pr-3 text-red-500 font-semibold">
                              {status.daysLeft !== null ? Math.abs(status.daysLeft) : "—"}
                            </td>
                            <td className="py-1.5 pr-3 text-muted-foreground">{m.notes || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {filteredMaterials.map(m => (
                  <MaterialRowItem key={m.id} material={m} onUpdate={updateMaterial} onDelete={deleteMaterial} disabled={!canEditMaterials}/>
                ))}
              </div>
            )}
